...
- Formal artifact schema contract: committed Draft 2020-12 **JSON Schema** files for the core artifacts (`run_summary`, `data_quality`, per-timeframe `composite_ohlcv` and `composite_orderbook_ladder`, `universe_summary`) under the packaged `crypto_composite.artifact_schemas` resource. `crypto-composite validate-artifacts --json-schema` validates a directory against them, and the committed sample artifacts are checked against the schemas in CI. Validation uses the optional `[schema]` extra (`jsonschema`), so the base install stays `requests`-only. A schema-stability policy (additive-only artifact fields at 1.0) is documented in `docs/SCHEMA_STABILITY.md`.
...
- Order-book record-isolation regression tests for every venue (`binance`, `okx`, `bybit`, `coinbase`, `kraken`, and both Gate book shapes): a malformed public level is skipped and the surrounding good levels survive. This locks the `parse_book_levels` invariant in on the book paths, which previously had record-isolation coverage only for candles and trades.
- Offline coverage for the `live_smoke` evidence writer (no network).
